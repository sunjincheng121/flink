...
package object ml {
  ...
  implicit class RichTable(table: Table) {
    def mapWithBcVariable[T, B, O: TypeInformation: ClassTag](
        broadcastVariable: Table
    )(fun: (T, B) => O): Table = {
    ...
    def filterWithBcVariable[T, B, O](broadcastVariable: Table)(fun: (T, B) => Boolean): Table = {
      val func = new BroadcastSingleElementFilterFunction(fun)
      table.as('value).join(broadcastVariable.as('broadcast))
        .where(func('value, 'broadcast))
    }

    def zipWithUUID[ID: TypeInformation, T: TypeInformation](): Table = {
      val func = new UUIDGenerateFunction[ID, T]
      table.as('t).select(func('t))
    }
