...
class SVMITSuite extends FlatSpec with Matchers with FlinkTestBase {
  ...
  it should "train a SVM" in {
    ...
    val trainingDS = tEnv.fromDataSet(env.fromCollection(Classification.trainingData))
  ...
  it should "make (mostly) correct predictions" in {
    ...
    val test = tEnv.fromDataSet(tEnv.toDataSet[LabeledVector](trainingDS).map(x => (x.vector, x.label)))

    svm.fit(trainingDS)

    val predictionPairs = svm.evaluate[(Vector, Double), Double](test)

    val absoluteErrorSum = tEnv.toDataSet[(Double, Double)](predictionPairs).collect().map{
  ...
  it should "be possible to get the raw decision function values" in {
    ...
    val test = tEnv.fromDataSet(env.fromElements(DenseVector(5.0, 5.0, 5.0)))

    val thresholdedPrediction = tEnv.toDataSet[(DenseVector, Double)](svm.predict[DenseVector, (DenseVector, Double)](test)).map(vectorLabel => vectorLabel._2).collect().head

    thresholdedPrediction should be (1.0 +- 1e-9)

    svm.setOutputDecisionFunction(true)

    val rawPrediction = tEnv.toDataSet[(DenseVector, Double)](svm.predict[DenseVector, (DenseVector, Double)](test)).map(vectorLabel => vectorLabel._2).collect().head
