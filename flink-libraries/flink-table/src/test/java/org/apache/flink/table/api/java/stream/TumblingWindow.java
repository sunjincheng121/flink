...
public class TumblingWindow {
    ...
    public static void main(String[] args) throws Exception {
        ...
        Table table = tableEnv.fromDataStream(
                ...
                "recordTime.rowtime," +
                "httpGetMessageCount," +
                "httpPostMessageCount," +
                "uplink," +
                "downlink," +
                "statusCode," +
                "statusCodeCount");

        Table windowTable = table.window(Tumble.over("1.hours").on("recordTime").as("w"))
                                 .groupBy("w, urlKey")
                                 .select("w.start,urlKey,uplink.sum,downlink.sum,httpGetMessageCount.sum,httpPostMessageCount.sum ");

        //table.printSchema();

        TableSink<Row> windowSink = new CsvTableSink("/Users/sunny/specific-website-code.csv",
                                                     ",",
                                                     1,
                                                     WriteMode.OVERWRITE);
