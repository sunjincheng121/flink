...
class OverWindowTest extends TableTestBase {
  ...
  def testProcTimeBoundedPartitionedRowsOver() = {
    val result = table
      .window(
        Over partitionBy 'c orderBy 'proctime preceding 2.rows following CURRENT_ROW as
          'w)
  ...
  def testProcTimeBoundedNonPartitionedRangeOver() = {
    val result = table
      .window(Over orderBy 'proctime preceding 10.second following CURRENT_RANGE as 'w)
  ...
  def testProcTimeBoundedNonPartitionedRowsOver() = {
    val result = table
      .window(Over orderBy 'proctime preceding 2.rows following CURRENT_ROW as 'w)
  ...
  def testProcTimeUnboundedNonPartitionedRowsOver() = {
    val result = table
      .window(Over orderBy 'proctime preceding UNBOUNDED_ROW following CURRENT_ROW as 'w)
      .select('c, 'a.count over 'w)
  ...
  def testRowTimeBoundedNonPartitionedRangeOver() = {
    val result = table
      .window(Over orderBy 'rowtime preceding 10.second following CURRENT_RANGE as 'w)
      .select('a, 'c.count over 'w)
  ...
  def testRowTimeBoundedNonPartitionedRowsOver() = {
    val result = table
      .window(Over orderBy 'rowtime preceding 2.rows following CURRENT_ROW as 'w)
      .select('c, 'a.count over 'w)
