...
class SqlITCase extends StreamingWithStateTestBase {
  ...
  def testBoundPartitionedEventTimeWindowWithRow(): Unit = {
    ...
    val t1 = env.fromCollection(data)
      .assignTimestampsAndWatermarks(new TimestampWithLatenessWatermark())
      .toTable(tEnv).as('a, 'b, 'c)

    tEnv.registerTable("T1", t1)

    val sqlQuery = "SELECT " +
      "c, a, sum(a) OVER (PARTITION BY c ORDER BY RowTime() ROWS BETWEEN 2 preceding AND CURRENT ROW)" +
      "from T1"
    ...
    val expected = mutable.MutableList("Hello,1,1", "Hello,2,3", "Hello,3,6", "Hello,4,9", "Hello,5,12",
