...
/**
  ...
  */
case class CharLength(child: PlannerExpression) extends PlannerUnaryExpression {
...
/**
  ...
  */
case class InitCap(child: PlannerExpression) extends PlannerUnaryExpression {
...
/**
  ...
  */
case class Lower(child: PlannerExpression) extends PlannerUnaryExpression {
  override private[flink] def resultType: TypeInformation[_] = STRING_TYPE_INFO
...
case class Trim(
  ...
  override private[flink] def validateInput(): ValidationResult = {
    trimMode match {
      case PlannerSymbolExpression(_: PlannerTrimMode) =>
...
object TrimConstants {
  val TRIM_DEFAULT_CHAR = PlannerLiteral(" ")
}

/**
  * Returns str with all characters changed to uppercase.
  */
case class Upper(child: PlannerExpression) extends PlannerUnaryExpression with InputTypeSpec {
...
/**
  ...
  */
case class FromBase64(child: PlannerExpression) extends PlannerUnaryExpression with InputTypeSpec {
...
/**
  ...
  */
case class ToBase64(child: PlannerExpression) extends PlannerUnaryExpression with InputTypeSpec {
...
/**
  ...
  */
case class LTrim(child: PlannerExpression) extends PlannerUnaryExpression with InputTypeSpec {
  override private[flink] def expectedTypes: Seq[TypeInformation[_]] = Seq(STRING_TYPE_INFO)
...
/**
  ...
  */
case class RTrim(child: PlannerExpression) extends PlannerUnaryExpression with InputTypeSpec {
