...
trait ImplicitExpressionOperations {
  ...
  def as(name: Symbol, extraNames: Symbol*) =
    Alias.apply(expr, name.name, extraNames.map(_.name).asJava)
  ...
  /**
    ...
    */
  def in(table: Table) = call(IN, Seq(expr, TableReference.apply(table.toString, table)))
  ...
  def over(alias: Expression): Expression = {
    UnresolvedOverCall.apply(expr, alias)
  }
...
trait ImplicitExpressionConversions {
  ...
  implicit class UnresolvedFieldExpression(s: Symbol) extends ImplicitExpressionOperations {
    def expr = UnresolvedFieldReference.apply(s.name)
  }
...
/**
  ...
  */
object Null {
  def apply(t: TypeInformation[_]): ENull = ENull.apply(t)
}
