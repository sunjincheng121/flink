...
import org.apache.flink.table.expressions.{Alias, Call, CurrentRange, CurrentRow, DistinctAggExpression, Expression, ExpressionUtils, ProctimeAttribute, RowtimeAttribute, ScalarFunctionDefinition, SymbolExpression, TableFunctionCall, TableFunctionDefinition, TableReference, TableSymbol, TimeIntervalUnit, TimePointUnit, DefaultExpressionVisitor, TrimConstants, TrimMode, TypeLiteral, UDAGGExpression, UnboundedRange, UnboundedRow, UnresolvedFieldReference, UnresolvedOverCall, Literal => ELiteral, Null => ENull}
...
trait ImplicitExpressionOperations {
  ...
  def over(alias: Expression): Expression = {
    new UnresolvedOverCall(expr, alias)
  }
  ...
  /**
    ...
    */
  def proctime = new ProctimeAttribute(expr)
...
trait ImplicitExpressionConversions {
  ...
  implicit val UNBOUNDED_ROW = new UnboundedRow()
  implicit val UNBOUNDED_RANGE = new UnboundedRange()

  implicit val CURRENT_ROW = new CurrentRow()
  implicit val CURRENT_RANGE = new CurrentRange()
  ...
  implicit class UnresolvedFieldExpression(s: Symbol) extends ImplicitExpressionOperations {
    def expr =  new UnresolvedFieldReference(s.name)
  }
  ...
  implicit def tableFunctionCall2Table(tfc: TableFunctionCall): Table = {
    ...
    new Table(
      ...
      LogicalTableFunctionCall(
        tfd.getFunc.getClass.getCanonicalName,
        tfd.getFunc,
  ...
  implicit class TableFunctionCallAlias(val tfc: TableFunctionCall) {
    def as(name: Symbol, extraNames: Symbol*): TableFunctionCall = {
      ...
      val alias = name.name +: extraNames.map(_.name)
      tfc.alias(alias.toArray)
      tfc
    }
  }
  ...
  implicit def symbol2FieldExpression(sym: Symbol): Expression =
    new UnresolvedFieldReference(sym.name)
...
/**
  ...
  */
object Null {
  def apply(t: TypeInformation[_]): ENull = new ENull(t)
}
